...
class SearchNewDestinationPromptData(BaseModel):
    to: str
    from_loc: str # mapped from "from"
    date: str
    travelers: int
    budget: float
    budget_range: Optional[List[str]] = None
    activities: Optional[List[str]] = None
    travel_style: Optional[str] = None
    duration: Optional[int] = None

def generate_new_search_destination_prompt(data: SearchNewDestinationPromptData) -> str:
    activities_list = ", ".join(data.activities) if data.activities else "general sightseeing and local culture"
    style = data.travel_style or "balanced"
    
    # Extract first string from list or default
    budget_type = data.budget_range[0] if data.budget_range and len(data.budget_range) > 0 else "standard"
    duration = f"{data.duration} days" if data.duration else "approximately 5-7 days"
    duration_number = data.duration or 7

    return f"""
        - **Total Budget:** ~{data.budget} INR (Target Upper Limit - {budget_type} tier)
        - **Travel Style:** {style}
        - **Interests:** {activities_list}
        ...
        **IMPORTANT: BUDGET REALISM RULE**
        - The "cost" field MUST be a realistic estimate for the destination and duration, considering the origin (**{data.from_loc}**) and destination (**{data.to}**).
        ...
        **IMPORTANT: DURATION RULE**
        The plan MUST be for exactly **{duration_number}** days. The "days" field and "suggested_itinerary" array length must match this number.
        ...
        **IMPORTANT: DIVERSITY RULE**
        ...
        4. "days": number (must be {duration_number}).
