  ...
  try {
    ...
    if (!country || !currency_code) {
      return next(
        createHttpError(
          400,
          'Country or Currency information is missing from user data!'
        )
      );
    }
    ...
    redis.get(redisKey, async (err, cacheData) => {
      ...
      if (cacheData) {
        ...
        try {
          generatePopularDest = (await generateRecommendation(
            prompt
          )) as string;

          await redis.set(
            redisKey,
            JSON.stringify(generatePopularDest),
            'EX',
            86400
          ); // Cache with expiry
        } catch (error) {
          return next(
            createHttpError(500, 'Error generating recommendation data.')
          );
        }
      }
      ...
      try {
        const result: string = generatePopularDest
          .replace(/```json|```/g, '')
          .replace(/\n/g, '')
          .trim();
        ...
        return next(
          createHttpError(500, 'Error parsing popular destinations response.')
        );
      }
    });
  } catch (error) {
