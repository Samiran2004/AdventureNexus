...
const updateProfile = async (
    ...
    try {
        ...
        if (!fullname && !gender && !preference && !country && !password && !(req as CustomRequestUpdateProfilePicture).file?.path) {
        ...
        if (fullname && fullname != checkUser.fullname) {
            ...
            try {
                checkUser.username = await generateRandomUserName(fullname) as string;
            } catch (error) {
        ...
        if ((req as CustomRequestUpdateProfilePicture).file?.path) {
            try {
                ...
                if (previousProfilePictureUrl) {
                    const publicId: string | undefined = previousProfilePictureUrl.split('/').pop()?.split('.')[0];
                    if (publicId) {
                        try {
