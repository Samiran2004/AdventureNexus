...
const userDelete = async (
  ...
  try {
    ...
    const checkUser: IUser | null = await User.findById(
    ...
    if (!checkUser) {
      ...
      const publicId: string | undefined = profilePictureUrl
        .split('/')
        .pop()
      ...
      // Send a mail
      const emailData: EmailData = emailTemplates.deleteUserEmailData(
        checkUser.fullname,
        checkUser.email
      );
      await sendMail(emailData, (error: Error | null) => {
        if (error) {
          return next(
            createHttpError(500, 'User deleted, but email sending failed!')
          );
        }
